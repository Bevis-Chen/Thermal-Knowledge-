...
> 我們在Fan PQ Curve 圖表裡面描述P是一個代表靜壓的參數，在固定轉速下風扇因為當下條件，靜壓與流量的關係。在此時所謂"最大靜壓"是因為風扇被完全封閉的擋板或阻抗等等，當風扇出口受到完全阻塞時，系統流量趨近於零。此時風扇仍持續對流體做功，因此可以建立最大的壓力升高；在該風扇的性能定義下，此狀態對應於 shutoff condition，並形成性能曲線上的高壓、低流量端點。
...
從PQ Curve的說明我們知道P 大部分是代表靜壓、也就是產生壓差的能力; System Impedance Curve的P 則是指壓力損失，代表系統內部的摩擦與局部阻力等等。**System Impedance Curve 本身並不描述風扇提供多少能量，而是描述在特定系統配置下，維持不同流量所需要克服的壓力損失。** 

當風扇安裝於系統中並開始運轉時，風扇對流體做功，使流體獲得壓力與速度相關的機械能；流體通過 Filter、Heatsink、Duct 等元件後，因摩擦與局部流動損失而消耗部分機械能。在適當的測試邊界與假設下，可以用：ΔP_fan ≈ ΔP_system，也就是描述前面所說的產生機械能與損耗機械能達成近似平衡。
...
P0 = 250 Pa（無流時的靜壓），k_f = 400 (Pa·s²/m^6)（風扇模型係數），k_s = 200 (Pa·s²/m^6)（系統阻抗係數），則 Q_working = √(250 / (400 + 200)) = √(250 / 600) ≈ 0.646 m³/s（換算約 2326 m³/h）
