...
靜壓 P_s（Static Pressure）是流體的壓力狀態；動壓 P_v（Velocity Pressure）則與流體速度所對應的動能項有關，可表示為 ½rou*v²。在低速、不可壓縮流體的簡化分析中，總壓 � 可表示為靜壓與動壓之和：�。

 靜壓 Ps（Static Pressure）：相對於流體平均動能以外的壓力，代表產生壓差以克服系統阻力的能力。
- 動壓 Pd（Dynamic Pressure）：1/2 * ρ * v²，與速度有關。
