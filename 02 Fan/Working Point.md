...
> 在Fan PQ Curve 圖表裡面 P (縱軸)的定義是 **Fan Static Pressure**，在固定轉速下風扇因為當下條件，靜壓與流量的關係。在此時所謂"最大靜壓"是因為風扇被完全封閉的擋板或阻抗等等，當風扇出口受到完全阻塞時，系統流量趨近於零。此時風扇仍持續對流體做功，因此可以建立最大的壓力升高；在該風扇的性能定義下，此狀態對應於 shutoff condition，並形成性能曲線上的高壓、低流量端點。


在本文

在本文採用的表示方式中，若 PQ Curve 的縱軸定義為 **Fan Static Pressure**，則：
> **PQ Curve 描述的是在某一固定轉速下，風扇在不同流量下所能建立的靜壓差。**

當風量降低時，風扇通常可以建立更高的壓力。當風扇出口逐漸受到限制，流量會下降；在理想化的極限情況下，流量趨近於零，對應到 **shutoff condition**。此時風扇仍在旋轉並對流體做功，可以建立較高的壓力升高，因此形成性能曲線上的高壓、低流量端點。需要注意：
> **Shutoff condition 並不代表風扇「沒有對流體做功」。**

風量接近零時，風扇內部仍可能存在複雜的循環流、旋轉流與損失，而且實際風扇的 shutoff 狀態可能伴隨顯著的非穩態流動。因此，實際性能曲線的端點仍應依風扇製造商與測試標準的定義解讀。
